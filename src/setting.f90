...
contains
  ...
  subroutine verify_setting(dim, setting)
    ...
    if (.not. &
         ...
         trim(setting%re_initialize_method) == 'minimize_lcao_error_matrix_suppress_damp' .or. &
  ...
  subroutine print_setting(setting)
    ...
    else if (setting%re_initialize_method == 'minimize_lcao_error_matrix_suppress_damp') then
      print *, 'matrix_suppress_constant: ', setting%suppress_constant
    end if
