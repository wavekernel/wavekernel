...
private
  ...
  public :: isSameNode
  public :: insertBefore
  public :: getChildValue ! Mizuho-IR
      ...
      do 
         ...
         if (associated(np % attributes)) then
            do i = 0, getLength(np%attributes) - 1
               ...
               newattr => createNode()
!               newattr%nodeName = getNodeName(attr) ! Mizuho-IR
               newattr%nodeName = attr%nodeName ! Mizuho-IR
!               newattr%nodeValue = getNodeValue(attr) ! Mizuho-IR
               newattr%nodeValue = attr%nodeValue ! Mizuho-IR
...
! Mizuho-IR
  ...
  function getChildValue(node)
    type(fnode), pointer :: node
    type(string)  :: getChildValue

    if( .not. associated(node) ) then
       getChildValue = ""
    else if( .not. associated(node%firstChild) ) then
       getChildValue = ""
    else if (getNodeType(node%firstChild) /= TEXT_NODE ) then
       getChildValue = ""
    else
       getChildValue = node%firstChild%nodeValue
    endif
  end function getChildValue
