...
!      (vect_out): = A (vect_in)
!
  subroutine matvec_mul_dens(vect_in, vect_out, mat_data)
    use M_qm_domain,     only : nval, atm_element  ! (unchanged)
!
    implicit none
...
!      (vect_out): = A (vect_in)
!
  subroutine matvec_mul_dens_dum(vect_in, vect_out, jjkset, jsv4jsk, booking_list_dstm, & 
&                                booking_list_dstm_len, mat_dstm)
    use M_qm_domain,     only : nval, atm_element  ! (unchanged)
!
    implicit none
    real(DOUBLE_PRECISION), intent(in)  :: vect_in(:)
...
!
    ...
    enddo
!
  end subroutine matvec_mul_dens_dum
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine calc_u_su_hu_dens(u,su,hu,norm_factor, mat_dens_h_s, ierr)
!
    implicit none
...
!     call matvec_mul_crs(u, su, irp, icol, val(:,2))
      call matvec_mul_dens(u, su, mat_dens_h_s(:,:,2))
!               ---> su : S |m_n> (non-normalized vector)
!
!
!     call matvec_mul_crs(u, hu, irp, icol, val(:,1))
      call matvec_mul_dens(u, hu, mat_dens_h_s(:,:,1))
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine calc_u_su_hu_dens_dum(u,su,hu,norm_factor, irp, icol, val, ierr )
!
    implicit none
    real(8),       intent(inout)  :: u(:)
    real(8),         intent(out)  :: su(:)
...
!        ---> hu : H|u_n> 
  ...
  end subroutine calc_u_su_hu_dens_dum
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine cg_s_mat_dens(b,x,eps,kend, mat_dens)
!
...
!
    beta = 0.0d0
!
    call matvec_mul_dens(x, ap, mat_dens)
...
!
      p(:) = r(:) + beta*p(:)
!
      call matvec_mul_dens  (p, ap, mat_dens)
