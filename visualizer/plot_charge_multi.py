...
def plot_charge_multi(charge_moments_with_labels, fig_path):
    ...
    for entry in charge_moments_with_labels:
        ...
        times = map(lambda t: t * kPsecPerAu, charge_moment["ts"])
        msds_x = map(lambda msd: msd[0], charge_moment["msds"])
        msds_x_ang = map(lambda x: x * kAngstrom2PerAu2, msds_x)
        ipratios = charge_moment["ipratios"]
        means_x = map(lambda mean: mean[0], charge_moment["means"])
        means_x_ang = map(lambda x: x * kAngstromPerAu, means_x)
        label_folded = ""
        max_len_in_line = 8
...
if __name__ == "__main__":
    ...
    plot_charge_multi(zip(charge_moments, legend_labels), args.fig_path)
