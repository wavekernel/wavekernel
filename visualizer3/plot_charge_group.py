    ...
    for charge in charge_group["charges_on_groups_all"]:
        if time_start <= charge["t"] and charge["t"] <= time_end:
            if i % stride == 0:
                ...
                if to_plot_msd_contribution:
                    key = "co"
                    charge[key] = [x * kAngstrom2PerAu2 for x in charge[key]]  #
                else:
                    key = "ch"
                pylab.bar(list(range(group_start, group_end + 1)),
                ...
                if is_log_mode:
                    ...
                    text_ys = [10.0 ** (exp_max - exp_diff * x) for x in [0.06, 0.14, 0.22]]
                else:
                    diff = max_val - min_val
                    text_ys = [diff * x for x in [0.94, 0.86, 0.78]]
