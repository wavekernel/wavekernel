...
def get_window_averaged_msds(ts, msdss, window_width):
    ...
    msdss_trans = list(map(list, list(zip(*msdss))))
...
def check_tss(group_to_tss):
    ...
    for tss in group_to_tss:
        ts_represent = tss[0]
        assert(all([ts == ts_represent for ts in tss[1 :]]))
...
def plot_charge_moment(grouped_charge_moments,
    ...
    group_to_tss = [[[t * kPsecPerAu for t in cm['ts']] for cm in cms] for cms in grouped_charge_moments]
    group_to_msdss = [[[x[msd_axis_num] * kAngstrom2PerAu2 for x in cm['msds']] for cm in cms] for cms in grouped_charge_moments]
    ...
    if window_width is None:
        ...
        group_to_avg_msds = list(map(avg, group_to_msdss))
    else:
        group_to_avg_ts = []
        group_to_avg_msds = []
...
def parse_charge_moment_calc_paths(ss):
    ...
    num_groups = len([s for s in ss if s == ':']) + 1
    grouped_paths = []
...
if __name__ == '__main__':
    ...
    if args.labels is None:
        ...
        grouped_labels = [g_str.split(',') for g_str in args.labels.split(':')]
    ...
    if args.group_to_avg_label is None:
        group_to_avg_label = ['group %d avg' % (i + 1) for i in range(len(grouped_paths))]
    else:
