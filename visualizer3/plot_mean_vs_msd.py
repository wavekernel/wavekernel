...
def plot(wavekernel_out, wavekernel_out_path, highlight_i, mean_min, mean_max, msd_min, msd_max, energy_min, energy_max, is_log, to_label, to_print, title, out_filename):
    ...
    indices = list(range(fst_filter, end_filter + 1))
    eigenvalues = condition['eigenvalues']
    eigenstate_msds = condition['eigenstate_msd_total']
    eigenstate_msds = [x * kAngstrom2PerAu2 for x in eigenstate_msds]
    eigenstate_means = condition['eigenstate_mean_z']
    eigenstate_means = [x * kAngstromPerAu for x in eigenstate_means]
    ...
    data_preplot_unzip = list(zip(*data_preplot))
    ...
    data_mainplot_unzip = list(zip(*data_mainplot))
    ...
    if not highlight_i is None:
        data_mainplot_highlighted = [d for d in data_mainplot if d[0] == highlight_i]
        means_highlighted = [d[2] for d in data_mainplot_highlighted]
        msds_highlighted = [d[3] for d in data_mainplot_highlighted]
