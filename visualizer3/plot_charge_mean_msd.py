...
import matplotlib
matplotlib.use("Agg")
import argparse, json, sys, re, pylab, os.path
...
def plot_diffusion_coef(ts, msds, label, time_start_diffusion, time_end_diffusion):
    ...
    xys = list(zip(ts, msds))[1:] # 初期ステップは error がでるため削除
...
def plot_charge_moment(charge_moment,
    ...
    msds = [x[msd_axis_num] * kAngstrom2PerAu2 for x in charge_moment['wave_center_msds']]
    means = [m[mean_axis_num] * kAngstromPerAu for m in charge_moment['means']]
    tb_energy_deviations = charge_moment['tb_energy_deviations']

    pylab.ylabel('Mean MSD ' + msd_axis + ' [$\AA^2$]', color='blue')
    pylab.plot(ts, msds, '+-', markeredgecolor='blue', label=('Mean MSD %s raw' % msd_axis),
