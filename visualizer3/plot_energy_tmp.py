...
def read_raw(fp):
    ...
    tb_energy = [e - tb_energy[0] for e in tb_energy]
    nl_energy = [e - nl_energy[0] for e in nl_energy]
    total_energy = [e - total_energy[0] for e in total_energy]
...
def plot(energy_calc, energy_min, energy_max, title, fig_path):
    if max(energy_calc['ts']) * kPsecPerAu < 0.001:
        ts = [t * kPsecPerAu * 1000 for t in energy_calc['ts']]
        pylab.xlabel('Time [fs]')
    else:
        ts = [t * kPsecPerAu for t in energy_calc['ts']]
