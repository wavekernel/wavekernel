...
if __name__ == '__main__':
    ...
    Xs = numpy.zeros((max_step_num, num_molecules, num_molecules))
        ...
        with open(os.path.join(dirname, f)) as fp:
            ...
            for step_num in range(len(xss)):
                for (atom2, orbital1, atom1, icohp) in xss[step_num]:
                    Xs[step_num][atom1 / molecule_size][atom2 / molecule_size] += icohp

    step_num = 0
    for X in Xs:
        X_symmetrized = (X + X.transpose()) / 2.0
        X_symmetrized_sparse = scipy.sparse.coo_matrix(X_symmetrized)
        scipy.io.mmwrite('bond_list_molecule_symmetrized_%06d.mtx' % step_num, X_symmetrized_sparse, \
                         comment='generated by read_bond_list.py. command: ' + ' '.join(sys.argv), \
                         field='real', precision=8)
        step_num += 1
